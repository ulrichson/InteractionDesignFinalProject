...
@interface KCUI : NSObject <KCSHKSharerDelegate, MFMailComposeViewControllerDelegate, KCYouTubeUploaderDelegate,KC_FBSessionDelegate, KC_FBDialogDelegate, KCLoginDismissedDelegate>
...
- (BOOL)shareVideo:(KCVideo *)video
        onFacebook:(BOOL)shareFacebook
           ...
           YouTube:(BOOL)shareYoutube
             Email:(BOOL)shareEmail
       withMessage:(NSString *)message
withDefaultMessage:(NSString *)defaultMessage
parentViewController:(UIViewController *)parentViewController
           delegate:(id <KCShareDelegate>)delegate;
